    ...
    public CartItemDto findById(Long id) {
        return toDto(
                cartItemRepository.findById(id)
                        .orElseThrow(() ->
                                new IllegalArgumentException("CartItem not found: " + id))
        );
    }
    ...
    private CartItem toEntity(CartItem cartItem, CartItemDto dto) {
        ...
        if (dto.cartId() != null) {
            Cart cart = cartRepository.findById(dto.cartId())
                    .orElseThrow(() ->
                            new IllegalArgumentException("Cart not found: " + dto.cartId()));
