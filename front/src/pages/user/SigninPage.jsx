...
const SigninPage = () => {
  ...
  const mergeLocalCart = async (token) => {
    ...
    try {
      ...
      await axios.post(`${BACKEND_URL}/api/cart/merge`, payload, {
        headers: {
          ...
          (res) => {
            if (res.authResponse) {
              ...
              window.FB.api(
                ...
                () => handleFacebookLoginToServer(accessToken),
              );
            }
          },
  ...
  const handleFacebookLoginToServer = async (accessToken) => {
    try {
      const res = await axios.post(
        `${BACKEND_URL}/api/users/auth/signinByFacebook`,
        {
          accessToken,
        },
      );
  ...
  const handleGoogleLogin = async (accessToken) => {
    try {
      const res = await axios.post(
        `${BACKEND_URL}/api/users/auth/signinByGoogle`,
        { accessToken },
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.post(`${BACKEND_URL}/api/users/auth/signin`, {
        email,
        password,
      });
      console.log(res.data);
        ...
        <div className="relative hidden lg:block">
          ...
          <div className="absolute inset-0 flex flex-col justify-center px-10 text-white">
            ...
            <p className="mt-4 text-base leading-7 text-gray-200">
              Đăng nhập để quản lý đơn hàng, lưu sản phẩm yêu thích và tiếp tục
